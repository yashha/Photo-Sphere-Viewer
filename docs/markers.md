...
Photo Sphere Viewer includes a powerful markers system allowing to define points of interest on the panorama with optional tooltip and description. Markers can be dynamically added/removed and you can react to use click/tap.
...
Markers can be added at startup with the `markers` option or after load with the methods of the `hud` property, check the [API Documentation](https://photo-sphere-viewer.js.org/api/PSV.HUD.html).
